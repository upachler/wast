...
use crate::w4::{SCREEN_SIZE, BLIT_2BPP, FRAMEBUFFER_ADDR};
...
/*
fn blit_impl<S,T>(sprite: S, target: T, x: i32, y: i32, width: u32, height: u32, flags: u32) 
where 
    S: Index<u8>,
    T: IndexMut<u8>
{
    ...
    let sx_mod_mask = 0xff >> (8-index_shift);

    let mut pixbuf;
    let mut pixbuf_len = 0;
    for sy in sy_r {

        let line_start_idx = (sx_r.start + sy * width as i32) >> index_shift;
        pixbuf = 
        for sx in sx_r {
            ...
            if pixbuf_len == 0 {
                let idx = line_start + (sx >> index_shift);
                pixbuf = sl.read(idx as u64).unwrap();

                let pix_in_byte = sx & sx_mod_mask;
                pixbuf_len = 8 - pix_in_byte;

                let pixbuf_initial_shift = (pix_in_byte >> (pixel_width-1));
                pixbuf = pixbuf >> pixbuf_initial_shift;
            }

            pixbuf & pixel_mask
        }
    }
st.fb.blit(target, sl.into(), x, y, width, height, flags)
...
{
    fn set_item_at(&mut self, offset: usize, item: T) {
       self.slice.index(offset as u64).write(item);
    }
}
...
fn pixel_width_of_flags(flags: u32) -> u32 {
    if flags & BLIT_2BPP != 0 {
        2
    } else {
        1
...
pub (crate) fn blit_sub(
    ...
    blit_sub_impl(&mut tgt, &src, x, y, width, height, src_x, src_y, stride, flags);
}

fn blit_sub_impl<S,T>(
    target: &mut T, 
    sprite: &S,
    x: i32,
    y: i32,
